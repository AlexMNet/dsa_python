...
# Dictionary will contain the sum as keys and the index that sum occurs at as values
# There may be an instance in which the first element equals the target
# So we will intitalize the dictionary with {0: -1}
...
def subarray_sum(nums, target):
    ...
    for i, num in enumerate(nums):
        curr_sum += num
        # Here we are looking to see if we have a number in sum_index we can remove to make curr_sum - target = 0
        if curr_sum - target in sum_index:
            # We have to add a + 1 to the starting index since we intialized sum_index with {0: 1}
...
#         print(
#             f"current_sum - target: {current_sum} - {target} = {current_sum - target}"
#         )
#         print(
#             f'{f"Is" if current_sum - target in sum_index else "Is NOT"} in sum_index'
#         )
#         print(sum_index)
#         print("***************************")
